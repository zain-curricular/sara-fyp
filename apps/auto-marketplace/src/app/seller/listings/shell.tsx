...
// ============================================================================
//
// My parts management page. Four-tab filter (All / Published / Drafts / Sold)
// + stats row. Each listing renders as a compact row with image placeholder,
// title, condition badge, price, status badge, and quick-action links.
...
export default function SellerListingsShell({ listings }: SellerListingsShellProps) {
	...
	return (
		<div container-id="seller-listings-shell" className="flex flex-col gap-6">
			...
			<header container-id="seller-listings-header" className="flex flex-wrap items-end justify-between gap-3">
				<div className="flex flex-col gap-1">
					<h1 className="text-3xl font-bold tracking-tight">My listings</h1>
					<p className="text-sm text-muted-foreground">Manage drafts and published parts.</p>
				</div>
			...
			{visible.length === 0 ? (
				...
				>
					...
					{tab === "all" ? (
						<Link href="/seller/listings/new" className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>
