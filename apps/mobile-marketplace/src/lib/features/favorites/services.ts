...
type FavoritesMeResponse =
	| {
			ok: true;
			data: FavoritesListPayload["items"];
			pagination: FavoritesListPayload["pagination"];
	  }
...
type RecentViewsResponse =
	| {
			ok: true;
			data: ViewedListPayload["items"];
			pagination: ViewedListPayload["pagination"];
	  }
	| { ok: false; error: string };

/** Authenticated GET /api/favorites/me for RSC. Returns `null` if there is no session or 401. */
...
/** Authenticated GET /api/me/recent-views for RSC. Returns `null` if there is no session or 401. */
export async function fetchMyViewedListings(
	...
	try {
		...
		if (!body.ok || !("data" in body)) {
			throw new Error("Recent views unavailable");
		}
		return {
			items: body.data,
			pagination: body.pagination,
