...
function buildChips(params: ListingsSearchParams): ActiveFilterChip[] {
	...
	const base = { ...params, platform: params.platform ?? "automotive", page: 1 as number | undefined };
...
export default function SearchShell({ listings, pagination, params }: SearchShellProps) {
	...
	const hrefBase = (p: number) =>
		`/search${toListingsApiQuery({ ...params, platform: params.platform ?? "automotive", page: p })}`;
		...
		>
			...
			<div container-id="search-main" className="flex min-w-0 flex-col gap-6">
				...
				{listings.length === 0 ? (
					...
					>
						<p className="text-sm font-medium">No parts match your filters.</p>
