// ============================================================================
// GET /api/warranty-claims/[id] — claimant, seller, or admin
// ============================================================================
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
	try {
		...
		const { data, error } = await getClaimDetailForParticipant(auth.user.id, idParse.data)
		if (error) {
		...
		Sentry.captureException(error, { extra: { route: 'GET /api/warranty-claims/[id]' } })
		console.error('UNEXPECTED: GET /api/warranty-claims/[id]', { error: serializeError(error) })
