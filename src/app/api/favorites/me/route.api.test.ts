// ============================================================================
// API integration tests — GET /api/favorites/me
// ============================================================================
...
describe.skipIf(!canRunSupabaseIntegrationTests)('GET /api/favorites/me', () => {
	...
	it('returns 200 with pagination and favorited listing', async () => {
		mockAuthenticatedUser(fx.regularUserId)
		const res = await GET(buildRequest('/api/favorites/me?page=1&limit=20'))
		expect(res.status).toBe(200)
		const body = await res.json()
		expect(body.ok).toBe(true)
		expect(body.pagination).toBeDefined()
		expect(body.pagination.total).toBeGreaterThanOrEqual(1)
		const ids = (body.data as { listing: { id: string } }[]).map((x) => x.listing.id)
